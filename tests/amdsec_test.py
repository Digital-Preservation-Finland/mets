...
def test_techmd():
    ...
    assert u.compare_trees(tech, ET.fromstring(xml)) == True
...
def test_digiprovmd():
    ...
    assert u.compare_trees(digiprov, ET.fromstring(xml)) == True

def test_amdsec():
    """test amdsec"""
    xml = '<mets:amdSec xmlns:mets="http://www.loc.gov/METS/"/>'
    amd = m.amdsec()
    assert u.compare_trees(amd, ET.fromstring(xml)) == True
